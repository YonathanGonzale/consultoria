...
# Reemplaza las clases existentes en tu models.py con estas versiones:

# ELIMINA las clases Vencimiento y Notificacion que agregaste al final de tu models.py
# Y reemplázalas con estas versiones SIN backrefs duplicados:
...
class Vencimiento(db.Model):
    ...
    id_propiedad = db.Column(db.Integer, db.ForeignKey('propiedad.id_propiedad'))
    estado = db.Column(db.String(50), default='vigente')  # vigente, vencido, renovado
    
    # NO definir backrefs aquí porque ya están definidos en Cliente y Propiedad
    notificaciones = db.relationship('Notificacion', backref='vencimiento', lazy=True, cascade='all, delete-orphan')
    ...
    def necesita_notificacion(self):
        """Verifica si necesita notificación (30 días o 7 días antes)"""
        dias = self.dias_hasta_vencimiento()
        if dias is None:
            return False
        
        # Verificar si ya se enviaron las notificaciones correspondientes
        notif_30 = any(n.tipo == '30_dias' for n in self.notificaciones)
        notif_7 = any(n.tipo == '7_dias' for n in self.notificaciones)
        
        # Necesita notificación de 30 días
        if dias <= 30 and dias > 7 and not notif_30:
            return '30_dias'
        
        # Necesita notificación de 7 días
        if dias <= 7 and dias >= 0 and not notif_7:
            return '7_dias'
        
        return False

    def __repr__(self):
        return f'<Vencimiento {self.tipo_documento} - {self.fecha_vencimiento}>'

class Notificacion(db.Model):
    __tablename__ = 'notificacion'
    
    id_notificacion = db.Column(db.Integer, primary_key=True)
    id_vencimiento = db.Column(db.Integer, db.ForeignKey('vencimiento.id_vencimiento'), nullable=False)
    tipo = db.Column(db.String(50), nullable=False)  # 30_dias, 7_dias
    fecha_envio = db.Column(db.Date, nullable=False)
    
    def __repr__(self):
        return f'<Notificacion {self.tipo} - {self.fecha_envio}>'