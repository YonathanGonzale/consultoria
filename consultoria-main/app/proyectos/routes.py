...
def _fill_project_from_form(proyecto, form_data):
    ...
    # Campos específicos BONO DE SERVICIOS AMBIENTALES
    proyecto.de_quien_compro = form_data.get('de_quien_compro') or proyecto.de_quien_compro
    proyecto.hectareas_bono = _parse_decimal(form_data.get('hectareas_bono'))
    proyecto.anio_bono = _parse_int(form_data.get('anio_bono'))
    proyecto.tipo_asociacion = form_data.get('tipo_asociacion') or proyecto.tipo_asociacion
    proyecto.fecha_emision_bono = _parse_date(form_data.get('fecha_emision_bono'))
    proyecto.fecha_vencimiento_bono = _parse_date(form_data.get('fecha_vencimiento_bono'))
    proyecto.pago_total_bono = _parse_decimal(form_data.get('pago_total_bono'))
    proyecto.hectareas_finanzas = _parse_decimal(form_data.get('hectareas_finanzas'))
    
    # Calcular precio por hectárea automáticamente
    proyecto.calcular_precio_por_hectarea()
...
def index():
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return render_template('proyectos/_results.html', **template_kwargs)

    return render_template('proyectos/list.html', **template_kwargs)
