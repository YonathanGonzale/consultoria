...
def seleccionar_periodo(id_cliente):
    ...
    # Si no hay proyectos, mostrar años recientes
    if not anos:
...
@login_required
def instituciones(id_cliente, ano):
    cliente = Cliente.query.get_or_404(id_cliente)
    
    # Obtener instituciones que tienen proyectos en este año para este cliente
    ...
    # Lista completa de instituciones disponibles
    todas_instituciones = [
        {
            "key": "MADES",
    ...
    # Marcar cuáles tienen proyectos en este año
    for inst in todas_instituciones:
        inst['tiene_proyectos'] = inst['key'] in instituciones_activas
    
    return render_template('clientes/instituciones.html', 
                         cliente=cliente, 
                         instituciones=todas_instituciones, 
                         ano=ano)

@bp.route('/<int:id_cliente>/ano/<int:ano>/institucion/<string:inst>')
@login_required
def institucion_detalle(id_cliente, ano, inst):
    cliente = Cliente.query.get_or_404(id_cliente)
    
    # Obtener tipos de trámite que existen para este cliente, año e institución
    ...
    return render_template('clientes/institucion_detalle.html', 
                         cliente=cliente, 
                         institucion=inst, 
                         tipos=todos_tipos, 
                         ano=ano)
